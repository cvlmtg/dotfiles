...
# Detect OS once to avoid redundant failed syscalls in helpers
_uname=$(uname)
...
iso_to_epoch() {
  ...
  # date -d is Linux-only; skip on macOS to avoid a guaranteed failed call
...
if [ -f "$cache_file" ]; then
  # macOS stat uses -f %m; Linux uses -c %Y — try native form first
...
if [ -n "$usage_data" ]; then
  ...
  # Single jq call extracts all usage fields at once
  {
    read -r five_hour_pct
    read -r five_hour_reset_iso
    read -r seven_day_pct
    read -r seven_day_reset_iso
  ...
  if [ -n "$five_hour_pct" ]; then
    ...
    if [ "$extra_enabled" = "true" ]; then
      read -r extra_used extra_limit < <(awk "BEGIN{printf \"%.2f %.2f\n\", $extra_used_raw/100, $extra_limit_raw/100}")
